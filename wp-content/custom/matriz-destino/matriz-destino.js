...
function populateA(i)
{
	...
	setCellValue22("ceu1_" + i, dotLL20[i]);
	setCellValue22("ceu2_" + i, dotLL60[i]);
	setCellValue22("ceu3_" + i, base22(dotLL20[i] + dotLL60[i]));

	setCellValue22("ter1_" + i, dotLL0[i]);
	setCellValue22("ter2_" + i, dotLL40[i]);
	setCellValue22("ter3_" + i, base22(dotLL0[i] + dotLL40[i]));
	//alert( "SKY: " + dotLL20[i] + " & " + dotLL60[i] + " & " + base22(dotLL20[i] + dotLL60[i]) );
	//alert( "EARTH: " + dotLL0[i] + " & " + dotLL40[i] + " & " + base22(dotLL0[i] + dotLL40[i]) );
	//alert( "SKYEARTH: " + base22( base22(dotLL20[i] + dotLL60[i]) + base22(dotLL0[i] + dotLL40[i]) ) );
}
...
/*
  ...
  sky.textContent = base22(dotLL20[i] + dotLL60[i]);
  const earth = document.querySelector("#earthpoint");
  earth.textContent = base22(dotLL0[i] + dotLL40[i]);
  const perspurpose = document.querySelector("#perspurpose");
  perspurpose.textContent = base22( base22(dotLL20[i] + dotLL60[i]) + base22(dotLL0[i] + dotLL40[i]) );
...
function setSvg_0()
{
	svg = document.getElementById("svg_0");
...
function getAge (element, date) 
{
	...
	if (value < 1) {
		...
		message = "Idade: " + parseInt(value) + " ano";
	} else {
		message = "Idade: " + Math.floor(value) + " anos";
	}
...
function calculate()
{
	...
	//var dateVal_2 = document.getElementById('inputDate2').value;
